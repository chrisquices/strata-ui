...
const base =
    'group flex w-full cursor-default select-none items-center h-control gap-2.5 rounded-medium px-2 py-1.5 text-xs transition-colors duration-100 focus-visible:outline-none';
...
<template>
  <MenubarItem :disabled="disabled" :class="[base, stateClass]" @select="onSelect">
    <component
        :is="icon"
        v-if="icon"
